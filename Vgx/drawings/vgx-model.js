
var sampleImageResource = "data:image/jpeg;base64,";

var pointDefinition = {
    id: "point1",
    figures: [
        {
            type: "path",
            data: "M 0.5,0.0 L 0.5,1.0 M 0.0,0.5 L 1.0,0.5"
        }
    ]
};

var dataSet = [
    {
        name: "Line", baseType: "Entity", members: ["startPoint", "endPoint"], properties: {
            startPoint: {
                "x": 20,
                "y": 190
            },
            endPoint: {
                "x": 220,
                "y": 60
            },
            stroke: "#3995fd",
            strokeWidth: 3
        }
    },
    {
        name: "Polyline", baseType: "Entity", members: ["points"], properties: {
            "points": [
                {
                    "x": 20,
                    "y": 190
                },
                {
                    "x": 180,
                    "y": 60
                },
                {
                    "x": 220,
                    "y": 140
                },
                {
                    "x": 120,
                    "y": 180
                }
            ],
            stroke: "#3995fd",
            strokeWidth: 3
        }
    },
    {
        name: "Arc", baseType: "Entity", members: ["endAngle", "isAntiClockwise", "radius", "startAngle"], properties: {
            insertPointX: 100,
            insertPointY: 150,
            radius: 80,
            startAngle: -115,
            endAngle: 20,
            isAntiClockwise: false,
            stroke: "#3995fd",
            strokeWidth: 3
        }
    },
    {
        name: "Rectangle", baseType: "Entity", members: ["width", "height"], properties: {
            insertPointX: 50,
            insertPointY: 75,
            width: 150,
            height: 95,
            stroke: "#3995fd",
            strokeWidth: 3,
            fill: "#9dd9ff"
        }
    },
    {
        name: "Square", baseType: "Entity", members: ["size"], properties: {
            insertPointX: 70,
            insertPointY: 75,
            size: 100,
            stroke: "#3995fd",
            strokeWidth: 3,
            fill: "#9dd9ff"
        }
    },
    {
        name: "Circle", baseType: "Entity", members: ["radius"], properties: {
            insertPointX: 120,
            insertPointY: 125,
            radius: 55,
            stroke: "#3995fd",
            strokeWidth: 3,
            fill: "#9dd9ff"
        }
    },
    {
        name: "Ellipse", baseType: "Entity", members: ["xRadius", "yRadius"], properties: {
            insertPointX: 120,
            insertPointY: 125,
            xRadius: 75,
            yRadius: 50,
            stroke: "#3995fd",
            strokeWidth: 3,
            fill: "#9dd9ff"
        }
    },
    {
        name: "Triangle", baseType: "Entity", members: ["point1", "point2", "point3"], properties: {
            point1: {
                "x": 30,
                "y": 155
            },
            point2: {
                "x": 180,
                "y": 70
            },
            point3: {
                "x": 210,
                "y": 170
            },
            stroke: "#3995fd",
            strokeWidth: 3,
            fill: "#9dd9ff"
        }
    },
    {
        name: "Quad", baseType: "Entity", members: ["point1", "point2", "point3", "point4"], properties: {
            point1: {
                "x": 40,
                "y": 115
            },
            point2: {
                "x": 180,
                "y": 70
            },
            point3: {
                "x": 210,
                "y": 140
            },
            point4: {
                "x": 140,
                "y": 170
            },
            stroke: "#3995fd",
            strokeWidth: 3,
            fill: "#9dd9ff"
        }
    },
    {
        name: "Polygon", baseType: "Entity", members: ["points"], properties: {
            "points": [
                {
                    "x": 20,
                    "y": 100
                },
                {
                    "x": 140,
                    "y": 130
                },
                {
                    "x": 180,
                    "y": 60
                },
                {
                    "x": 220,
                    "y": 140
                },
                {
                    "x": 120,
                    "y": 180
                }
            ],
            stroke: "#3995fd",
            strokeWidth: 3,
            fill: "#9dd9ff"
        }
    },
    {
        name: "QuadraticCurve", baseType: "Entity", members: ["points", "controlPoints", "isClosed"], properties: {
            "controlPoints": [
                {
                    "x": 161,
                    "y": 63
                },
                {
                    "x": 190,
                    "y": 120
                },
                {
                    "x": 187,
                    "y": 171
                },
                {
                    "x": 112,
                    "y": 175
                },
                {
                    "x": 70,
                    "y": 195
                },
                {
                    "x": 50,
                    "y": 141
                },
                {
                    "x": 49,
                    "y": 55
                }
            ],
            "points": [
                {
                    "x": 131,
                    "y": 61
                },
                {
                    "x": 177,
                    "y": 95
                },
                {
                    "x": 189,
                    "y": 147
                },
                {
                    "x": 150,
                    "y": 173
                },
                {
                    "x": 92,
                    "y": 183
                },
                {
                    "x": 59,
                    "y": 168
                },
                {
                    "x": 49,
                    "y": 113
                }
            ],
            stroke: "#3995fd",
            strokeWidth: 3,
            fill: "#9dd9ff"
        }
    },
    {
        name: "CubicCurve", baseType: "Entity", members: ["points", "controlPoints1", "controlPoints2", "isClosed"], properties: {
            "points": [
                {
                    "x": 36,
                    "y": 116
                },
                {
                    "x": 96,
                    "y": 109
                },
                {
                    "x": 150,
                    "y": 113
                },
                {
                    "x": 203,
                    "y": 115
                }
            ],
            "controlPoints1": [
                {
                    "x": 50,
                    "y": 72
                },
                {
                    "x": 67,
                    "y": 150
                },
                {
                    "x": 165,
                    "y": 67
                },
                {
                    "x": 151,
                    "y": 189
                }
            ],
            "controlPoints2": [
                {
                    "x": 119,
                    "y": 77
                },
                {
                    "x": 136,
                    "y": 155
                },
                {
                    "x": 233,
                    "y": 73
                },
                {
                    "x": 12,
                    "y": 187
                }
            ],
            stroke: "#3995fd",
            strokeWidth: 3,
            fill: "#9dd9ff"
        }
    },
    {
        name: "Pie", baseType: "Entity", members: ["endAngle", "isAntiClockwise", "radius", "startAngle"], properties: {
            insertPointX: 100,
            insertPointY: 150,
            radius: 80,
            startAngle: -100,
            endAngle: 20,
            isAntiClockwise: false,
            stroke: "#3995fd",
            strokeWidth: 3,
            fill: "#9dd9ff"
        }
    },
    {
        name: "Donut", baseType: "Entity", members: ["endAngle", "endRadius", "isAntiClockwise", "startAngle", "startRadius"], properties: {
            insertPointX: 120,
            insertPointY: 130,
            startRadius: 30,
            endRadius: 60,
            startAngle: -115,
            endAngle: 205,
            isAntiClockwise: false,
            stroke: "#3995fd",
            strokeWidth: 3,
            fill: "#9dd9ff"
        }
    },
    {
        name: "Path", baseType: "Entity", members: ["figures", "fillRule"], properties: {
            insertPointX: 0,
            insertPointY: 0,
            figures: [
                { type: "rect", insertPointX: 21, insertPointY: 105, width: 35, height: 35 },
                { type: "rect", insertPointX: 63, insertPointY: 105, width: 35, height: 35 },
                { type: "rect", insertPointX: 105, insertPointY: 105, width: 35, height: 35 },
                { type: "path", data: "M 220, 120.19L 147, 72.276L 147, 168.104L 220, 120.19 Z " }
            ],
            stroke: "#3995fd",
            strokeWidth: 3,
            fill: "#9dd9ff"
        }
    },
    {
        name: "Text", baseType: "Entity", members: ["fontFamily", "fontSize", "text", "textAlign", "textBaseline"], properties: {
            insertPointX: 20,
            insertPointY: 115,
            fontFamily: "Arial",
            fontSize: 28,
            text: "Vector Graphics",
            textAlign: "left",
            textBaseline: "middle",
            stroke: "transparent",
            strokeWidth: 0,
            fill: "#3995fd"
        }
    },
    {
        name: "Image", baseType: "Entity", members: ["height", "src", "width"], properties: {
            insertPointX: 20,
            insertPointY: 70,
            width: 194,
            height: 103,
            stroke: "#3995fd",
            strokeWidth: 3,
            fill: "#9dd9ff",
            src: sampleImageResource
        }
    }/*,
    {
        name: "Point", baseType: "Symbol", members: [], properties: {
            insertPointX: 120,
            insertPointY: 140,
            stroke: "#3995fd",
            strokeWidth: 3
        }
    }*/
];



function createEntityInfoGroup(data) {

    var group = new Vgx.Group();

    var fontFamily = "Open Sans";
    //var fontFamily = "Arial";
    var titlePadding = 10;

    var txtTitle = new Vgx.Text();
    txtTitle.insertPointX = titlePadding;
    txtTitle.insertPointY = 26;
    txtTitle.fontFamily = fontFamily;
    txtTitle.fontSize = 24;
    txtTitle.fill = "#464646";
    txtTitle.text = data.name;
    var titleBounds = txtTitle.getBounds();

    var rectTitle = new Vgx.Rectangle();
    rectTitle.cornersRadius = 6;
    rectTitle.insertPointX = 0;
    rectTitle.insertPointY = 0;
    //rectTitle.width = titlePadding + titleBounds.width + titlePadding;
    rectTitle.setBinding("width", () => {
        return titlePadding + txtTitle.getBounds().width + titlePadding;
    });
    rectTitle.height = 34;
    rectTitle.fill = "#CECECE";
    group.children.add(rectTitle);

    group.children.add(txtTitle);

    if (data.baseType) {
        var txtInherit = new Vgx.Text();
        //txtInherit.insertPointX = titlePadding + titleBounds.width + titlePadding + 8;
        txtInherit.setBinding("insertPointX", () => {
            return titlePadding + txtTitle.getBounds().width + titlePadding + 8;
        });
        txtInherit.insertPointY = 26;
        txtInherit.fontFamily = fontFamily;
        txtInherit.fontSize = 18;
        txtInherit.fill = "#464646";
        txtInherit.text = ": " + data.baseType;
        group.children.add(txtInherit);
    }

    if (!data.abstract) {
        var rectDrawing = new Vgx.Rectangle();
        rectDrawing.cornersRadius = 6;
        rectDrawing.insertPointX = 0;
        rectDrawing.insertPointY = 40;
        rectDrawing.width = 235;
        rectDrawing.height = 170;
        rectDrawing.fill = "#F3F3F3";
        group.children.add(rectDrawing);

        var typeCtor = Vgx[data.name];
        var entity = new typeCtor();
        for (var n in data.properties) {
            if (!data.properties.hasOwnProperty(n))
                continue;
            if (n in entity) {
                entity[n] = data.properties[n];
            }
        }
        switch (data.name) {
            case "Polyline":
            case "Polygon":
                if ("points" in data.properties) {
                    for (var p = 0; p < data.properties.points.length; p++) {
                        entity.points.add(data.properties.points[p]);
                    }
                }
                break;
            case "QuadraticCurve":
                if ("points" in data.properties) {
                    for (var p = 0; p < data.properties.points.length; p++) {
                        entity.points.add(data.properties.points[p]);
                    }
                }
                if ("controlPoints" in data.properties) {
                    for (var p = 0; p < data.properties.controlPoints.length; p++) {
                        entity.controlPoints.add(data.properties.controlPoints[p]);
                    }
                }
                break;
            case "CubicCurve":
                if ("points" in data.properties) {
                    for (var p = 0; p < data.properties.points.length; p++) {
                        entity.points.add(data.properties.points[p]);
                    }
                }
                if ("controlPoints1" in data.properties) {
                    for (var p = 0; p < data.properties.controlPoints1.length; p++) {
                        entity.controlPoints1.add(data.properties.controlPoints1[p]);
                    }
                }
                if ("controlPoints2" in data.properties) {
                    for (var p = 0; p < data.properties.controlPoints2.length; p++) {
                        entity.controlPoints2.add(data.properties.controlPoints2[p]);
                    }
                }
                break;
            case "Group":
                if ("children" in data.properties) {
                    for (var c = 0; c < data.properties.children.length; c++) {
                        var childElement = DrawingLoader.loadChildElement(data.properties.children[c]);
                        if (childElement != null)
                            entity.children.add(childElement);
                    }
                }
                break;
            case "Path":
                if ("figures" in data.properties) {
                    for (var f = 0; f < data.properties.figures.length; f++) {
                        var figure = data.properties.figures[f];
                        switch (figure.type) {
                            case "arc":
                                entity.addArc(figure.insertPointX, figure.insertPointY, figure.radius, figure.startAngle || 0, figure.endAngle || Math.PI * 2, figure.optClockwise || null);
                                break;
                            case "rect":
                                entity.addRect(figure.insertPointX, figure.insertPointY, figure.width, figure.height);
                                break;
                            case "ellipse":
                                entity.addRect(figure.insertPointX, figure.insertPointY, figure.radiusX, figure.radiusY, figure.rotation || 0, figure.startAngle || 0, figure.endAngle || Math.PI * 2, figure.optClockwise || null);
                                break;
                            case "path":
                                entity.addFigure(figure.data);
                                break;
                        }
                    }
                }
                break;
        }
        group.children.add(entity);
    }

    var rectProperties = new Vgx.Rectangle();
    rectProperties.cornersRadius = 6;
    rectProperties.insertPointX = data.abstract ? 0 : 241;
    rectProperties.insertPointY = 40;
    rectProperties.width = data.abstract ? 396 : 155;
    rectProperties.height = 170;
    rectProperties.fill = "#E3E3E3";
    group.children.add(rectProperties);

    var y = 62;
    if (data.baseType != null && data.baseType != "" && data.baseType != "Object") {
        var txtInheritedProp = new Vgx.Text();
        txtInheritedProp.insertPointX = data.abstract ? 10 : 251;
        txtInheritedProp.insertPointY = y;
        txtInheritedProp.fontFamily = fontFamily;
        txtInheritedProp.fontSize = 16;
        txtInheritedProp.fill = "#2f7ed7";
        txtInheritedProp.text = data.baseType + " properties +";
        group.children.add(txtInheritedProp);
        y = 85;
    }
    
    var stepY = 20;
    for (var i = 0; i < data.members.length; i++) {
        var txtProp = new Vgx.Text();
        txtProp.insertPointX =  data.abstract ? 10 : 251;
        txtProp.insertPointY = y + (stepY * i);
        txtProp.fontFamily = fontFamily;
        txtProp.fontSize = 16;
        txtProp.fill = "#464646";
        txtProp.text = data.members[i];
        group.children.add(txtProp);
    }


    return group;
}


var groupHorizontalOffset = 450;
var groupVerticalOffset = 260;

var baseTypesGroup = new Vgx.Group();
baseTypesGroup.insertPointX = 0;

var abstractEntityGroup = createEntityInfoGroup({ name: "VgxObject", baseType: "", abstract: true, members: ["drawing", "handle"] });
//drawing.addChild(abstractEntityGroup);
baseTypesGroup.children.add(abstractEntityGroup);

abstractEntityGroup = createEntityInfoGroup({ name: "Drawable", baseType: "VgxObject", abstract: true, members: [/*"appearanceDirty", "geometryDirty", "positionDirty",*/ "visible"] });
abstractEntityGroup.insertPointX = groupHorizontalOffset;
//drawing.addChild(abstractEntityGroup);
baseTypesGroup.children.add(abstractEntityGroup);

abstractEntityGroup = createEntityInfoGroup({ name: "Entity", baseType: "Drawable", abstract: true, members: ["insertPointX", "insertPointY", "shadow", "stroke", "strokeWidth", "transform"] });
abstractEntityGroup.insertPointX = groupHorizontalOffset * 2;
//drawing.addChild(abstractEntityGroup);
baseTypesGroup.children.add(abstractEntityGroup);

drawing.addChild(baseTypesGroup);

var maxColumns = 5;
for (var i = 0; i < dataSet.length; i++) {
    var entityGroup = createEntityInfoGroup(dataSet[i]);
    var column = i % maxColumns;
    var row = (i - column) / maxColumns; 
    entityGroup.insertPointX = groupHorizontalOffset * column;
    entityGroup.insertPointY = groupVerticalOffset * (row + 1);
    drawing.addChild(entityGroup);
}

drawing.background = "#FFFFFF";